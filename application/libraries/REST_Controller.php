...
abstract class REST_Controller extends MY_Controller {
    ...
    protected function _detect_api_key()
    {
        ...
        {
					 //We are adding sha1 hash to our key so it can't be stolen as easy
					  $key = sha1($key);
    ...
    protected function _check_limit($controller_method)
    {
        ...
        $time_limit = (isset($this->methods[$controller_method]['time']) ? $this->methods[$controller_method]['time'] : 60);
        ...
        else
        {
            ...
            {
	            // Tick the counter so we still count against limit
	            $this->rest->db
	                ->where('uri', $limited_uri)
	                ->where('api_key', $api_key)
	                ->set('hour_started', time())
